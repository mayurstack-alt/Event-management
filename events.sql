CREATE TABLE users (
    ...
    password VARCHAR(255) NOT NULL, -- Increased length for bcrypt hashes
...
-- Migration commands for existing database:
-- ALTER TABLE users DROP COLUMN IF EXISTS role;
-- ALTER TABLE users ADD COLUMN role VARCHAR(20) CHECK (role IN ('Organizer', 'Participant'));
-- ALTER TABLE users ALTER COLUMN password TYPE VARCHAR(255);

-- Optional: Clear existing users if you want to start fresh with proper password hashing
-- DELETE FROM users;
