...
if (!userId) {
  ...
  alert("Access denied. This page is only for Organizers. You are logged in as " + role + ".");
  window.location.href = "index.html"; // or "participant.html" if role is Participant
}
...
addEventForm.addEventListener('submit', async (e) => {
  ...
  try {
    const res = await fetch('http://localhost:3000/api/events', {
      method: 'POST',
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ user_id: userId, title, date, venue, description })
    });
    const out = await res.json();
    if (out.success) {
      addEventForm.reset();
